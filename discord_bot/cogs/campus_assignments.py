...
from discord_bot.cogs._campus_common import _clean, _guild_timezone, _optional_reminder
...
class AssignmentCog(
    commands.GroupCog,
    group_name="과제",
    group_description="과제와 제출 기한을 관리합니다.",
):
    ...
    @app_commands.command(name="추가", description="과제와 제출 기한을 등록합니다.")
    @app_commands.rename(
        course="과목",
        title="과제명",
        due="마감",
    ...
    @app_commands.describe(
        course="과목명",
        title="과제명",
        due="예: 2026-08-10 23:59",
        notes="선택 입력",
        remind_before="예: 30m, 1h, 1d. 필요 없으면 비우기",
    )
    async def add(
        self,
        ...
        reminder_text = f" · 알림 `#{reminder_id}`" if reminder_id else ""
    ...
    @app_commands.command(name="보기", description="마감이 가까운 과제를 확인합니다.")
    @app_commands.rename(include_completed="완료포함")
    ...
    @app_commands.command(name="완료", description="과제를 완료 처리합니다.")
    @app_commands.rename(assignment_id="번호")
    ...
    @app_commands.command(name="삭제", description="과제를 삭제합니다.")
    @app_commands.rename(assignment_id="번호")
