...
class ExamCog(
    ...
    @app_commands.command(name="추가", description="시험 일정을 등록합니다.")
    @app_commands.rename(
        course="과목",
        title="시험명",
        when="일시",
    ...
    @app_commands.describe(
        when="예: 2026-10-20 10:00",
        remind_before="예: 1h, 1d, 1w. 필요 없으면 비우기",
    )
    async def add(
        self,
    ...
    @app_commands.command(name="보기", description="다가오는 시험을 확인합니다.")
    ...
    @app_commands.command(name="삭제", description="시험 일정을 삭제합니다.")
    @app_commands.rename(exam_id="번호")
